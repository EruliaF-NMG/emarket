/*
 * @Author: Nisal Madusanka(EruliaF) 
 * @Date: 2018-12-25 10:56:38 
 * @Last Modified by: Nisal Madusanka(EruliaF)
 * @Last Modified time: 2018-12-30 18:05:04
 */


///coreReducer Keys
export const setInputValueKEY="@coreReducer/SET_INPUTS";
export const setBulkInputValuesKEY="@coreReducer/SET_BULK_INPUTS";
export const setFormErrorsKEY="@coreReducer/SET_INPUTS_ERRORS";
export const unSetFormErrorsKEY="@coreReducer/UNSET_INPUTS_ERRORS";
export const setDBStatusKEY="@coreReducer/SET_TO_DB_STATUS";
export const setApiDataToStoreKey="@coreReducer/SET_API_DATA_TO_STORE";

///coreUIReducer Keys
export const preLoaderKey="@coreReducer/PAGE_PRE_LOARDER";
export const manageModelKEY="@coreReducer/SHOW__HIDE_MODEL";

///AuthReducer Keys
export const checkAuthLoginKey="@authReducer/SET_AUTH_DATA";
export const logoutAuthUserKey="@authReducer/LOGOUT_USER";
export const setProfileKey="@authReducer/SET_PROFILE_DATA"